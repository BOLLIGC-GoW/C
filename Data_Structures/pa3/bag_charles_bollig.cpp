...
//Overloaded [] Operator    
    bag::value_type& bag::operator[] (size_type index)
    {
        assert(index <= this->size());
	assert(index <= DEFAULT_CAPACITY);
	if(index <= this->size())
		return data [index];	
    }


//Non-Member palindrome function
   bag* palindrome(const bag& b1)
    {
	
	//Create new object and assign values from the original bag (size, etc.)
	//Pointer to palindromed object
	bag *bagPtr =  new bag(b1);

	//Make the 'used' value twice as the size to make room for the palindrome
	*(bagPtr) += b1;	

	int middle = bagPtr->size()/2;

	//Use for loop to assign data's reversed values to the new object's data 
	//array after mid point 
	for(int i = 0; i > middle; i++)
	{	
		bagPtr[(middle+1)+i] = bagPtr[middle -i];
	}


	
	//Return the palindrome dynamic array
	return bagPtr;
    }
